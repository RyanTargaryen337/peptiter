...
| D-035 | 2026-07-05 | The sign-in donor's full-page instance lands as `/account.html`: centered account card on Paper (72vh stage), same stripped treatment as the modal (D-032), linked from every footer. Form logic refactored to one class-scoped implementation serving both modal and page; `novalidate` keeps errors in the mono bracket register instead of browser bubbles. Home-hero and floating CTAs keep opening the modal; the page is the linkable destination. | Owner pasted the donor with "implement"; the modal already covered the card, the page was the missing instance. | Locked |
| D-036 | 2026-07-05 | Third regulatory category tag added: `[SUPPLEMENT / BLEND]` for amino-acid/vitamin blends (SHB, LC526, LC216, NAD+, B12, glutathione, HA, Lemon Bottle, melatonin) that are neither GLP-1/hormone prescription meds nor research chemicals. Fixed-vocabulary, monochrome-only, mandatory on the card — same rules as D-025; tagging a vitamin as a research chemical would be false. | Owner-supplied product list (2026-07-05) includes non-peptide blends; owner chose a neutral third tag over mislabeling. Amends D-025's two-tag vocabulary. | Locked |
| D-037 | 2026-07-05 | Owner-supplied reconstitution data built out: a `/reconstitution.html` reference (standard ratios, U-100 conversion chart, exceptions, handling, supplies), each guide's Reconstitution section filled from it, the calculator selector expanded to the full catalog (optgroups, data-driven category + reconstitution hint), and the library grown to ~59 products across 7 groups. Reconstitution/unit-conversion are owner-sourced and citable; protocol doses and cycle lengths stay empty sourced slots (CONTEXT.md §8) — the owner supplied ratios, not protocols. Catalog generated from a committed script (build-catalog.py). | Owner supplied the full product list + reconstitution ratios (2026-07-05). | Locked |
| D-038 | 2026-07-05 | Account-gated protocol tracker (`/tracker.html`): donor skeleton (pasted "Peptide Bench"). Vial roster with live reconstitution math, dose logging + injection-site rotation, next-due / projected-finish / beyond-use logic, a 4-cell status strip, and a 7-day agenda. Skin: Paper/White tool surface, 1px Ink borders, 4/24 radius tiers, mono data + label registers, teal only at active/conversion. Donor's dark theme, 5-colour status palette, gradients, glow and lucide icons stripped; status reads as bracketed mono tags (teal for active/due, Ink otherwise, Ink-fill for warnings). Gate: reuses the account-card; since accounts are idea-stage, creating the demo account (localStorage flag) unlocks it and data persists to the browser only, stated plainly. User-entered protocol figures are the user's own log — not site-fabricated content, so CONTEXT.md §8 is not engaged. | Owner: "add a tracker for protocols and peptides, only accessible to those who create accounts." Realises CONTEXT.md's progress-log feature. | Locked |
